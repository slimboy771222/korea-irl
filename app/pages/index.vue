...
<template><main>
<section class="relative isolate min-h-[560px] overflow-hidden bg-[var(--brand-ink)] text-white md:min-h-0" @mouseenter="isHovered=true" @mouseleave="isHovered=false"><div class="absolute inset-0" aria-hidden="true"><img v-for="(slide,index) in heroSlides" :key="slide.path" :src="slide.url" :alt="slide.label" :loading="index===0?'eager':'lazy'" :fetchpriority="index===0?'high':'low'" class="hero-image absolute inset-0 h-full w-full object-cover transition-opacity ease-in-out" :class="[index===activeSlide?'opacity-100':'opacity-0',prefersReducedMotion?'duration-0':'duration-1000']" :style="{'--hero-desktop-position':slide.desktopPosition,'--hero-mobile-position':slide.mobilePosition}"/></div><div class="absolute inset-0 bg-gradient-to-r from-black/80 via-black/50 to-black/15" aria-hidden="true"/><div class="relative mx-auto max-w-7xl px-5 pb-12 pt-14 md:py-28"><p class="mb-2 text-sm font-medium uppercase tracking-[0.18em] md:mb-3">Korea, in real life</p><h1 class="max-w-2xl text-4xl font-semibold leading-tight md:text-6xl">Travel Korea<br>Like You Actually<br>Live Here.</h1><p class="mt-4 max-w-xl text-white/80 md:mt-5">Real tips. Local spots. Routes that make sense.</p><div class="mt-6 flex max-w-xl rounded-xl bg-white p-2 text-[var(--brand-ink)] shadow-xl md:mt-8"><input class="min-w-0 flex-1 bg-transparent px-3 outline-none" placeholder="Where do you want to go?"/><button class="rounded-lg bg-[var(--brand-ink)] px-5 py-3 text-sm text-white">Search</button></div></div><div class="absolute bottom-5 right-5 z-10 flex gap-2 md:bottom-7 md:right-7"><button v-for="(slide,index) in heroSlides" :key="slide.path" type="button" class="h-2 rounded-full bg-white/70 transition-all" :class="index===activeSlide?'w-5 bg-white':'w-2 hover:bg-white'" :aria-label="`Show ${slide.label}`" :aria-pressed="index===activeSlide" @click="selectSlide(index)"/></div></section>
